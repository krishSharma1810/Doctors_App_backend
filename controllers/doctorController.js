...
submitDoctorAvailability = async (req, res) => {
  ...
  try {
    ...
    if (!findFirst) {
      ...
      const savedData = await DoctorAvailability.findOneAndUpdate(
        { doctor_id: doctorId },
        { ...req.body },
        { new: true }
      );
...
getCurrentBookings = async (req, res) => {
  if (req.data.user.role == "doctor") {
    ...
    try {
      const patientBookings = await bookings.aggregate([
        {
          $match: {
            $and: [
              { bookingDate: { $eq: date } },
              { doctor_id: new mongoose.Types.ObjectId(id) },
            ],
          },
        },
        {
...
addDetailsToPatient = async (req, res) => {
  if (req.data.user.role == "doctor") {
    try {
      ...
      if (!data) return res.status(200).json({ status: 209 });
